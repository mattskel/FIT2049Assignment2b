...
void FlyingCamera::Update(float timestep)
{
	...
	m_heading += m_input->GetMouseXDelta() * m_rotationSpeed * timestep;
	m_pitch += m_input->GetMouseYDelta() * m_rotationSpeed * timestep;
	...
	// At this point, our lookAt vector is still relative to the origin
