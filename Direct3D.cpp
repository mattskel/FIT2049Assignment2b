...
bool Direct3D::Initialise(int width, int height, HWND windowHandle, bool fullscreen, bool vsync)
{
	...
	D3D_FEATURE_LEVEL featureLevel[] = { D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_0 };	//We are going to target the DirectX 11 feature level
	...
	// Disable writes to the depth buffer. We'll use this for rendering transparent objects like particles.
	depthStencilDescription.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;

	if (FAILED(m_device->CreateDepthStencilState(&depthStencilDescription, &m_depthStencilWriteDisabledState)))
	{
		return false;
	}
	...
	if (FAILED(m_device->CreateBlendState(&blendStateDescription, &m_alphaBlend)))
	{
		return false;
	}
	...
	// Create the blend state using the description.
	if (FAILED(m_device->CreateBlendState(&blendStateDescription, &m_normalBlend)))
	{
		return false;
	}

	m_deviceContext->OMSetBlendState(m_normalBlend, NULL, 0xFFFFFFFF);
...
void Direct3D::TurnOnBlending()
{
	m_deviceContext->OMSetBlendState(m_alphaBlend, NULL, 0xFFFFFFFF);
}

void Direct3D::TurnOffBlending()
{
	m_deviceContext->OMSetBlendState(m_normalBlend, NULL, 0xFFFFFFFF);
}

void Direct3D::TurnOnZWrite()
{
	m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
}

void Direct3D::TurnOffZWrite()
{
	m_deviceContext->OMSetDepthStencilState(m_depthStencilWriteDisabledState, 1);
}